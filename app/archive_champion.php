...
<?php
	...
	if (isset($_SESSION['login']))
	{
		...
		HISTORIA ZAKŁADÓW MISTRZÓW</br></br></p>';
		
		$login = $_SESSION['login'];
		$database = mysqli_connect('localhost', 'root', '', 'zaklady') or die("Could not connect");
		$query = "SELECT DISTINCT * FROM Zaklad_mistrz WHERE id_gracz = '$login' ORDER BY id";
		...
		{
			echo '<tr>';
			echo '<td>' . $row['id_druzyna'] . '</td>';
			echo '<td>' . $row['stawka']. '</td>';
			echo '<td>' . $row['kurs'] . '</td>';
			if($row['rezultat'] == 0) echo '<td>SEZON W TRAKCIE</td>';
