...
<?php
	...
	if (isset($_SESSION['login']))
	{
		...
		$query = "SELECT DISTINCT * FROM Pilkarz ORDER BY kurs_strzelca LIMIT 30";
		$data = mysqli_query($database, $query) or die("Query failed");
		$login = $_SESSION['login'];
		
		while($row = mysqli_fetch_array($data))
		{
			$kurs = $row['kurs_strzelca'];
			$kurs = $kurs*10; 
			echo '<p><a href="king.php?id_pilkarz=' . $row['id'] .  '&amp;id_gracz=' . $login .  '&amp;imie=' . $row['imie'] .  '&amp;nazwisko=' . $row['nazwisko'] . '&amp;kurs=' . $kurs . '">' . $row['imie'] . " " . $row['nazwisko'] . '</a></p>';
		}
